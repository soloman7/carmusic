...
- **网络电台（v3.5~v3.7）**：RadioBrowser 全球 **5.8 万有效电台全量内置**（local-first，中国 2,000+ 台 + 全球全量，搜索覆盖每一台），流式分批导入带进度条；**分类浏览（v3.7）**：国家网格（中国首位，国旗+台数，按实际收听量排序）→ 中国按省份（34 省别名容错）/ 他国按类型（流行/摇滚/新闻/古典/爵士/老歌/舞曲/财经/谈话/文艺/交通）→ 每类 top 100；收藏 / 本省（GPS 推荐）/ 搜索四屏，LIVE 收听条带流量估算
...
- **横屏双栏布局**：封面+控制在左，歌词在右
- **驾驶模式**：GPS 测速自动进入（进入：连续 3 个样本 >5km/h；退出：连续 10 个样本 <3km/h，堵车蠕行不误退；无速度字段的定位样本跳过不误判，隧道丢星超时后状态不横跳），极简大按钮界面，行驶中盲按
- **播放会话恢复**：队列 + 曲目 + 进度落盘（Room），DiLink 杀进程后点播放/方向盘任意媒体键（含播放键）即续播"停车前听到哪"；Android Auto 端可直接点播收藏/历史
...
- **每周自动维护**（v3.8 收藏红线）：启动时检测距上次维护超 7 天，自动校验歌单有效性（平台确认无曲目的进黑名单自动隐藏，网络故障不误判，黑名单每轮重新探测、恢复即自动移出）、剔除**播放历史**中的死链歌曲（连续两个维护周期探测失败才删，删除前先过网络连通性哨兵；探测超时不计入死链证据，弱网误判不删数据；维护全程避让播放）、清理 30 天前的歌词缓存；**收藏歌曲永不自动删除**（原平台下架的歌播放时跨平台 fallback 换源续播）；可在设置页手动触发
...
├── maintenance/
│   ├── ContentCleaner.kt   # 每周维护：歌单有效性黑名单 / 历史死链清理(收藏永不删) / 过期歌词
│   └── ProbeGateway.kt     # 清理探测 seam:三态证据契约(可播/确认无源/网络不可信)
