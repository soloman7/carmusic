...
| 2026-09-17 | v3.8.0 (code 32) | 用户要求排查"每周定时更新歌单"并保证歌单有效+收藏不受影响。排查:功能已存在(v3.2 ContentCleaner,启动+7 天门禁),但两处违背红线——①死链清理范围含**收藏**(两周期确认后连收藏一起删,且判据 NoFallback 比播放语义严:判定死 ≠ fallback 后不能播,删除即误伤)②getPlaylistTracks 把网络失败吞成空列表(已知技术债),清理器把网络抖动误读为"平台确认歌单无效"拉黑一周。整改:①清理范围收窄到**仅历史**,收藏永不探测永不删(挂账修剪只保留历史 id)②getPlaylistTracks 失败统一上抛 SourceUnavailableException(UI 错误通道接活,清理器既有 catch 接住→证据不可信不拉黑),清理器走 ProbeGateway/PlaybackYield seam(AppContainer 适配)③8 条清理语义单测(收藏两周期不删/历史两周期才删/同 trackId 删历史留收藏/异常不动账本/黑名单三态自愈/哨兵失败零触碰/旧挂账修剪/恢复出账)④设置页文案对齐。门禁:单测 96/0 ✅ 回归 exit 0 ✅ SHA256 3ffb9273…b5db ✅ | ZCode |
| | | | |
